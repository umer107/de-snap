...
$(document).ready(function () {
    ...
    jQuery(".dropdown").not(".noDk .dropdown").dropkick({
        mobile: true
    });
    ...
    $('.editInfo').on('click', function () {
        ...
        if ($(this).parent('form').attr('id') == 'frm_customer') {
			$('#frm_customer #individual_rating li a').on('click', function () {
			   var parent = $(this).parent('li');
			   if(parent.prevAll('li').size() > 0){
				   $('#frm_customer #individual_rating #cusindividualrating').val(parent.prevAll('li').size());
				   var ratingPercent = 20*parent.prevAll('li').size();
				   $('#frm_customer #individual_rating .star_rating60').width(ratingPercent+'%');
			   }
			});
			
			$('#frm_customer #referral_rating li a').on('click', function () {
			   var parent = $(this).parent('li');
			   if(parent.prevAll('li').size() > 0){
				   $('#frm_customer #referral_rating #cusreferralrating').val(parent.prevAll('li').size());
				   var ratingPercent = 20*parent.prevAll('li').size();
				   $('#frm_customer #referral_rating .star_rating60').width(ratingPercent+'%');
			   }
			});
            $('.cstmrDetails').addClass('editMode');
			$(this).closest('.editCustomerForm').find('.uploadCustomerPhoto').show();
            //$('.cstmrPhoto').find('.uploadify-button-text').text("Upload Photo")
        } else {
			$('#frm_partner #individual_rating li a').on('click', function () {
			   var parent = $(this).parent('li');
			   if(parent.prevAll('li').size() > 0){
				   $('#frm_partner #individual_rating #partindividualrating').val(parent.prevAll('li').size());
				   var ratingPercent = 20*parent.prevAll('li').size();
				   $('#frm_partner #individual_rating .star_rating60').width(ratingPercent+'%');
			   }
			});
			
			$('#frm_partner #referral_rating li a').on('click', function () {
			   var parent = $(this).parent('li');
			   if(parent.prevAll('li').size() > 0){
				   $('#frm_partner #referral_rating #partreferralrating').val(parent.prevAll('li').size());
				   var ratingPercent = 20*parent.prevAll('li').size();
				   $('#frm_partner #referral_rating .star_rating60').width(ratingPercent+'%');
			   }
			});
			$('.cstmrDetails').addClass('editMode');
			$(this).closest('.editCustomerForm').find('.uploadCustomerPhoto').show();
    ...
    $('body').on('click', '.lightBoxClick', function () {
    	openLightBox($(this).attr('data-popup'));
    });
    ...
    $('.lightBoxTitle .closePopup').on('click', function () {
        ...
        $(this).parents('.lightBox').addClass('closeAnimation').removeClass('showAnimation');
		setTimeout(function(){
			//alert();
			thisElm.parents('.lightBox').hide().removeClass('closeAnimation');
		},1000)
...
function openLightBox(popup) {
    $("#" + popup).show().addClass('newUI').addClass('showAnimation');
	if($("#" + popup).find('.lightBoxBottom').length==0){
		$("#" + popup).append('<div class="lightBoxBottom"></div>');
	}
    $("#" + popup).find();
    var winHeight = parseInt($(window).height());
    var thisHeight = parseInt($("#" + popup + ' > .lightBoxMid > .lightBoxContent').outerHeight()) + 38;
    var selectHeight = parseInt(thisHeight) + 30;

    if (selectHeight > winHeight) {
        var realheight = parseInt(winHeight) - 78;
        $("#" + popup + ' >  .lightBoxMid > .lightBoxContent').css({top: '58px'});
        $("#" + popup).find('.lightBoxTitle').css({top: '20px'});
    } else if (selectHeight <= winHeight) {
        var popupHeight = (parseInt(winHeight) - parseInt(thisHeight)) / 2;
        var topPos = parseInt(popupHeight) + 38;
        $("#" + popup + ' > .lightBoxMid > .lightBoxContent').css({'top': topPos + 'px'});
        $("#" + popup).find('.lightBoxTitle').css({top: popupHeight + 'px'});
    }
}
...
function getCount(gridType, customerId) {
    ...
    $.post(url, data, function (response) {
        ...
        if (gridType == 'leads') {// lead details
            $('#leadsCountGlobal').html(countList['leads']);
        }*/
        if (gridType == 'opportunities') {// customer details
            if (customerId) {
                $('#oppCustomerCountGlobal').html(countList['opportunities']);
				$('#invoiceQuotes').html(countList['invoice']);
				$('#ordersCount').html(countList['orders']);
				$('#jobsCount').html(countList['jobs']);
				$('#email_tab').html(countList['emails']);
            } /*else {
        ...
        if (gridType == 'gridType') {// left panel
            $('#customersCountGlobal').html(countList['customers']);
            $('#leadsCountGlobal').html(countList['leads']);
            $('#opportunitiesCountGlobal').html(countList['opportunities']);
            $('#suppplierCountGlobal').html(countList['suppliers']);
...
$(document).on('blur', 'input[data-numeric]', function (e) {
	var value = $(this).val();
...
function searchGridsData(prefix) {
    ...
    if (keyword != '' && keyword != 'undefined') {
        if (gridType == 'leads') {
	    var formData = $('#frm_filter_flead').serialize();
            if (selectedValue != '') {
                var url = '/editgridview';
                ...
                $.post(url, data, function (response) {
                    ...
                    getLeads(recordsPerPage, columnList, keyword, formData);
                });
            } else {
                getLeads(recordsPerPage, '', keyword, formData);
            }
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    customersList(recordsPerPage, columnList, keyword);
                });
            } else {
                customersList(recordsPerPage, '', keyword);
            }
        } else if (gridType == 'opportunities') {
			var formData = $('#frm_filter_fpportunity').serialize();
            if (selectedValue != '') {
                var url = '/editgridview';
                var data = 'id=' + selectedValue;
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getOpportunities(recordsPerPage, columnList, keyword, formData);
                });
            } else {
                getOpportunities(recordsPerPage, '', keyword, formData);
            }
        } else if (gridType == 'suppliers') {
			var formData = $('#frm_filter_fsupplier').serialize();
            if (selectedValue != '') {
                var url = '/editgridview';
                var data = 'id=' + selectedValue;
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getSuppliers(recordsPerPage, columnList, keyword, formData);
                });
            } else {
                getSuppliers(recordsPerPage, '', keyword, formData);
            }
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;					
                    getDiamonds(recordsPerPage, columnList, keyword, formData);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getWeddingrings(recordsPerPage, columnList, keyword);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getEngagementrings(recordsPerPage, columnList, keyword);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getEarrings(recordsPerPage, columnList, keyword);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getPendants(recordsPerPage, columnList, keyword);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getMiscellaneous(recordsPerPage, columnList, keyword);
                });
            } else {
            ...
            if (selectedValue != '') {
                ...
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getChain(recordsPerPage, columnList, keyword);
                });
            } else {
                getChain(recordsPerPage, '', keyword);
            }
        } else if (gridType == 'inventoryjobpackets') {
            if (selectedValue != '') {
                var url = '/editgridview';
                var data = 'id=' + selectedValue;
                $.post(url, data, function (response) {
                    var setting = JSON.parse(response);
                    var columnList = setting.columns_list;
                    getJobPacketsFromGridView(recordsPerPage, columnList, keyword);
                });
            } else {
                getJobPackets(recordsPerPage, keyword);
            }
        }
    } else {
...
function searchInputClear(prefix) {
    ...
    var customer_id = $('#customer_id').val();
    var selectedValue = '';
    if ($('#' + prefix + 'hiddenSelectGridView').length) {
        selectedValue = $('#' + prefix + 'hiddenSelectGridView').val();
    }
    var recordsPerPage = $('#pageSizeGrabing').val();
    ...
    if (gridType == 'leads') {
        ...
        if (selectedValue != '') {
            ...
            $.post(url, data, function (response) {
                var setting = JSON.parse(response);
                var columnList = setting.columns_list;
                customersList(recordsPerPage, columnList);
            });
        } else {
        ...
        if (selectedValue != '') {
            ...
            $.post(url, data, function (response) {
                var setting = JSON.parse(response);
                var columnList = setting.columns_list;
                getDiamonds(recordsPerPage, columnList, keyword, customer_id);
            });
        } else {
			var formData = $('#frm_filter_fiamond').serialize();
            getDiamonds(recordsPerPage, '', keyword, formData);
...
function cancelButtonProperty(formId, popupId) {
	...
	$('#' + formId+' .uploadedImgDisplay').html('');
	$('#' + formId+' #image').val('');
	$('#' + formId+' #invoice').val('');
	$('#' + formId+' .uploadedImages').html('');
	$('#' + formId+' #order_attachment').val('');
    $('#' + popupId + ' .closePopup').click();
}

function closeButtonProperty(formId) {
    $('#' + formId)[0].reset();
	$('#' + formId+' .uploadedImgDisplay').html('');
	$('#' + formId+' #image').val('');
	$('#' + formId+' #invoice').val('');
...
function addAdditionalRow(settingOptions, shapeOptions, gemtypeOptions, hideState) {
   ...
   newCell.innerHTML = '<input type="text" value="" class="inputTxt" data-numeric="yes" name="size[]" id="size'+temprow+'">';
   
   var newCell = mainRow.insertCell(5);
   newCell.innerHTML = '<input type="text" value="" class="inputTxt" data-numeric="yes" name="totalcarat[]" id="totalcarat'+temprow+'">';   
...
function unlinkGalleryImage(imgFullData, DivId, frmId, hiddenValueHolderId, imgId){
	...
	var order_attachments = 0;
	if(frmId == "frm_order"){
		order_attachments = 1;
	}
	if (confirm("Are you sure?")) {
		if(imgValidId > 0){
			data = {'fileFullData':imgFullData, 'imgId':imgId, 'order_attachments':order_attachments};
		} else {
			data = {'fileFullData':imgFullData, 'order_attachments':order_attachments};
		}
			...
			success: function(response){
				if(response){
					...
					if(frmId == "frm_order"){
						...
						imgName = imgFullData.replace('/order_attachment/', '');
