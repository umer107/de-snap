function getChain(recordsPerPage, columnList, keyword){
	$after = $('#jqxChain').prev('#bindAfterThis');
	...
	$('#jqxChain').remove();
    $after.after('<div class="formTable manageMembers" id="jqxChain"></div>');
	...
	if(columnList == '' || columnList == undefined){
		...
		if ($('#chain_columnList').length) {
			columnList = $('#chain_columnList').val();
		} else {
			/* Default columns */
			columnList = 'consign_button,inventory_status_name,inventory_status_reason,stock_code,style,cad_code,metal_type,price,supplier_name,inventory_type,inventory_tracking_status,inventory_tracking_reason,reserve_time,reserve_notes';
		}
	}
	$('#chain_columnList').val(columnList);
	...
	if ($('#chain_cellClick').length) {
		$('#jqxChain').bind('cellclick', window[$('#chain_cellClick').val()]);
	} else {
