	...
	$.ajax({
			...
			success: function(response){
				...
				if(taskData.task_assigned_fullname){
					var colorCode = '';
					if(taskData.color != null){
						colorCode = 'style="background-color:#'+taskData.color+'"';
					}
					$('.dropValue').html('<span '+colorCode+'>'+taskData.task_assigned_shortname+'</span>'+taskData.task_assigned_fullname);
					thirdColHtml += '<span class="assignTo" '+colorCode+'>'+taskData.task_assigned_shortname+'</span>';
				} else {
			...
			success: function(response){
				...
				$.each(taskHistoryData, function (){
					...
					if(this.task_assigned_fullname != null){
						var colorCode = '';
						if(this.color != null){
							colorCode = 'style="background-color:#'+this.color+'"';
						}
						var assignName = '<span class="assignTo" '+colorCode+'>'+this.task_assigned_shortname+'</span> '+this.task_assigned_fullname;
					}
					...
					divData = fillRightBlockWithHistory(this.id, this.task_id, this.metadata, this.data, this.task_created_fullname, this.createdDate, this.files, assignName, category_title, subject_title, priority_title, this.color);
...
function fetchLatestTaskHistory(id){
	...
	$.ajax({
			...
			success: function(response){
				var taskHistoryData = JSON.parse(response);
				if(!taskHistoryData.color)
					taskHistoryData.color = '';
					
				//alert(taskHistoryData.color);
				//for(i=0; i < taskHistoryData.length; i++){
					var divData = '';
					var assignName = '';
					var category_title = '';
					...
					if(taskHistoryData.task_assigned_fullname != null){
						var colorCode = '';
						if(taskHistoryData.color != null){
							colorCode = 'style="background-color:#'+taskHistoryData.color+'"';
						}
						var assignName = '<span class="assignTo" '+colorCode+'>'+taskHistoryData.task_assigned_shortname+'</span> '+taskHistoryData.task_assigned_fullname;
					}
					...
					divData = fillRightBlockWithHistory(taskHistoryData.id, taskHistoryData.task_id, taskHistoryData.metadata, taskHistoryData.data, taskHistoryData.task_created_fullname, taskHistoryData.createdDate, taskHistoryData.files, assignName, category_title, subject_title, priority_title, taskHistoryData.color);
...
function fillRightBlockWithHistory(historyId, taskId, columnTitle, columnValue, createdBy, createdDate, attachments, assignName, category_title, subject_title, priority_title, colorCode){
	...
	if(columnTitle == 'comment'){
		...
		var htmlData = '<div class="commentData" id="div_comment_'+historyId+'">';
		htmlData += '<div class="editDrop"><span class="editTaskBtn"></span><ul><li onclick="prepareEditComment('+historyId+');">Edit</li><li onclick="deleteComment('+historyId+');">Delete</li></ul></div><div class="cb"></div>';
		if (colorCode) {
			colorCodeIn = 'style="background-color:#'+colorCode+'"';
			htmlData += '<div class="commentedOne"><span '+colorCodeIn+'>'+initial+'</span>'+createdBy+'</div>';
		}
		htmlData += '<p class="prevPost" id="text_comment_'+historyId+'">'+columnValue+'</p><textarea id="input_comment_'+historyId+'" class="commentTextarea" onblur="editComment('+historyId+', $(\'#input_comment_'+historyId+'\').val());" style="display:none;">'+columnValue+'</textarea><div class="attachedImages">';
