function getPendants(recordsPerPage, columnList, keyword){
	$after = $('#jqxPendant').prev('#bindAfterThis');
	...
	$('#jqxPendant').remove();
    $after.after('<div class="formTable manageMembers" id="jqxPendant"></div>');
	...
	if(columnList == '' || columnList == undefined){
		...
		if ($('#pendant_columnList').length) {
			columnList = $('#pendant_columnList').val();
		} else {
			/* Default columns */
	...
	$('#pendant_columnList').val(columnList);
	...
	if ($('#pendant_cellClick').length) {
		$('#jqxPendant').bind('cellclick', window[$('#pendant_cellClick').val()]);
	} else {
