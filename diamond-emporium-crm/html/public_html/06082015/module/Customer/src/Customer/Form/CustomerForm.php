...
class CustomerForm extends Form {
	...
	public function __construct($objServiceManager) {
		try{
			...
			$twitter->setAttributes(array('id' => 'googleplus', 'class' => 'inputTxt editViewField', 'placeholder' => 'URL Link'));
			$this->add($twitter);			
			
			/*$cancel = new \Zend\Form\Element\Button('cancel');
			$cancel->setLabel('Cancel');
			$cancel->setValue('Cancel');
			$cancel->setAttributes(array('id' => 'cancel', 'class' => 'cmnBtn cancelBtn'));
			$this->add($cancel);
			
			$save = new \Zend\Form\Element\Button('save');
			$save->setLabel('Save');
			$save->setValue('Save');
			$save->setAttributes(array('id' => 'save', 'class' => 'cmnBtn', 'onclick' => 'saveLead($(\'#frm_lead\'));'));
			$this->add($save);*/
		}catch(\Exception $e){
