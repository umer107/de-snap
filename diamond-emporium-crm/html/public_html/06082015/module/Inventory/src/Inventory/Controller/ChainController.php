...
class ChainController extends AbstractActionController
{
	...
	public function chaindetailsAction(){
		try{
			...
			return array('recordsPerPage' => $config['recordsPerPage'], 'viewData' => $ChainData, 'fetchChainStyleOptions' => $objLookupTable->fetchChainStyleOptions(), 'metalTypesLookup' => $objLookupTable->fetchMetalTypeOptions(),'ConsignData' => $ConsignData);
		}catch(Exception $e){
			\De\Log::logApplicationInfo ( "Caught Exception: " . $e->getMessage () . ' -- File: ' . __FILE__ . ' Line: ' . __LINE__ );
		}
	}
