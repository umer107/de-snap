...
class EngagementringTable
{
	...
	public function fetchAll($limit, $offset, $keyword = null,  $sortdatafield = null, $sortorder = null, $filter = null)
	{
		try{
			...
			$select->from(array('engage' => 'de_inventory_engagement_rings'))
				   ->columns(array('id', 'cad_code', 'band_width', 'band_thickness', 'metal_thickness', 'halo_width', 'halo_thickness', 'setting_height', 'description', 'invoice', 'price', 'created_date', 'stock_code'))
			...
			if(!empty($sortdatafield) && !empty($sortorder)){
				...
				elseif($sortdatafield == 'band_width')
					$select->order("engage.band_width $sortorder");
				elseif($sortdatafield == 'band_thickness')
					$select->order("engage.band_thickness $sortorder");
				...
				elseif($sortdatafield == 'metal_thickness')
					$select->order("engage.metal_thickness $sortorder");
	...
	public function fetchEngagementringDetails($id)
	{
		try{
			...
			$select->from(array('engage' => 'de_inventory_engagement_rings'))
				   ->columns(array('id', 'cad_code', 'band_width', 'band_thickness', 'metal_thickness', 'description', 'image', 'invoice', 'price', 'ring_type_lookupid' => 'ring_type', 'metal_type_lookupid' => 'metal_type', 'profile_type_lookupid' => 'profile', 'supplier_id', 'halo_width', 'halo_thickness', 'setting_height', 'stock_code'))
				   ->join(array('drt' => 'de_engagementring_type_lookup'), 'engage.ring_type = drt.id', array('ring_type'), 'left')
				   ->join(array('dmtl' => 'de_metal_type_lookup'), 'engage.metal_type = dmtl.id', array('metal_type'), 'left')
				   ->join(array('dpl' => 'de_profile_lookup'), 'engage.profile = dpl.id', array('profile'), 'left')
				   ->join(array('dhsl' => 'de_head_settings_lookup'), 'engage.head_settings = dhsl.id', array('head_title'), 'left')
