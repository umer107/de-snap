function getWeddingrings(recordsPerPage, keyword){
	if(keyword){
	...
	function InitGrid() {
        ...
        $('#bindAfterThis').after('<div class="formTable manageMembers" id="jqxWeddingring"></div>');
	}
	// prepare the data
	var source =
	{
	...
	$("#jqxWeddingring").jqxGrid(
	{
		...
		columns: [
			...
			{ text: 'Inventory <br /> Status', type: 'string', datafield: 'inventory_status_name', width:'10%'},
			{ text: 'Reason', type: 'string', datafield: 'inventory_status_reason', width:'12%'},
			...
			{ text: 'Inventory <br /> Type', type: 'string', datafield: 'inventory_type', width:'10%'},
			...
			{ text: 'Tracking ID', type: 'string', datafield: 'tracking_id', width:'12%'},
			...
			{ text: 'Ring Type', type: 'string', datafield: 'ring_type', width:'10%'},
			{ text: 'CAD <br /> Code', type: 'string', datafield: 'cad_code', width:'10%'},
			{ text: 'Metal <br /> Type', type: 'string', datafield: 'metal_type', width:'8%'},
			{ text: 'Profile', type: 'string', datafield: 'profile', width:'10%'},
			{ text: 'Band <br /> Width', type: 'string', datafield: 'band_width', width:'8%'},
			{ text: 'Band <br /> Thickness', type: 'string', datafield: 'band_thickness', width:'10%'},
			{ text: 'Finish', type: 'string', datafield: 'finish', width:'8%'},
			{ text: 'Fit Option', type: 'string', datafield: 'fit_option', width:'10%'},
			{ text: 'Metal <br /> Thickness', type: 'string', datafield: 'metal_thickness', width:'10%'},
			{ text: 'Price', type: 'string', datafield: 'price', width:'8%'},
			{ text: 'Record <br /> Owner', type: 'string', datafield: 'owner_name', width:'13%'},
	...
	$("#jqxWeddingring").bind("sort", function (event) {
	   ...
	   //$.get(sortUrl, function(source){
	    	//$("#jqxWeddingring").jqxGrid('databind', source);
	    	$("#jqxWeddingring").jqxGrid('updatebounddata', 'filter');
	   //});
	});
...
function getWeddingringsFromGridView(recordsPerPage, columnList, keyword){
	...
	var consign = function(row, columnfield, value, defaulthtml, columnproperties){
		...
		var html = '';
		if(datarow)
			html = '<a href="javascript:;" data-popup="consignItem" class="cmnBtn lightBoxClick fl" onclick="openConsignForm('+datarow.id+', \'weddingring\')">Consign</a>';
		return html;
	};
	...
	if(colList.length>6){
		for(i = 0; i < colList.length; i++){
			...
			if(colList[i] == 'metal_thickness'){
				gridColumnList.push({ text: 'Metal <br /> Thickness', type: 'string', datafield: 'metal_thickness', width:'10%'});
			}
			if(colList[i] == 'price'){
		...
		$("#jqxWeddingring").jqxGrid({columns:gridColumnList});
	}else{
		...
		for(i = 0; i < colList.length; i++){
			...
			if(colList[i] == 'metal_thickness'){
				gridColumnList.push({ text: 'Metal Thickness', type: 'string', datafield: 'metal_thickness', width:colWidthData+'%'});
			}
			if(colList[i] == 'price'){
	...
	if(colList.length == gridColumnList.length){
		...
		//$("#jqxWeddingring").next('.pagerHTML').html($('#pagerjqxWeddingring'));
		...
		if(errors == 0){
			...
			$.post(url, data, function(response){
				if(response > 0){
					...
					$('#metal_thickness_weddring').text($('#metal_thickness').val());
