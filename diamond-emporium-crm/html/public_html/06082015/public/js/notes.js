function getNotes(recordsPerPage, type, id, displayActions){
	...
	var action = function (row, column, value) {
		var noteId = $("#jqxNotes").jqxGrid('getrowdata', row).id;
			return '<div style="text-overflow: ellipsis; text-align: left; overflow: hidden; margin-top: 11px; margin-right: 2px; margin-left: 4px; padding-bottom: 2px;"><a href="javascript:;" onclick="editNote('+noteId+');" class="gridLink">Edit</a> | <a href="javascript:;" class="gridLink" onclick="deleteNote('+noteId+', \''+type+'\');">Delete</a></div>';
	}
	...
	var FilteredData = new Array();
		...
		FilteredData.push({ text: 'Action', cellsrenderer: action});
