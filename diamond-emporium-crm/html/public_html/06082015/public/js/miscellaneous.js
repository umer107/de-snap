function getMiscellaneous(recordsPerPage, keyword){
	if(keyword){
	...
	function InitGrid() {
        ...
        $('#bindAfterThis').after('<div class="formTable manageMembers" id="jqxMiscellaneous"></div>');
	}
	// prepare the data
	var source =
	{
	...
	$("#jqxMiscellaneous").jqxGrid(
	{
		...
		columns: [
			...
			{ text: 'Title', type: 'string', datafield: 'title', width:'10%' },
			{ text: 'Description', type: 'string', datafield: 'description', width:'10%' },
...
function getMiscellaneousFromGridView(recordsPerPage, columnList, keyword){
	...
	if(colList.length>5){
		...
		$("#jqxMiscellaneous").jqxGrid({columns:gridColumnList});
	}else{
