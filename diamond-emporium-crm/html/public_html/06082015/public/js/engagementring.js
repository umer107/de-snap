function getEngagementrings(recordsPerPage, keyword){
	if(keyword){
	...
	function InitGrid() {
        ...
        $('#bindAfterThis').after('<div class="formTable manageMembers" id="jqxEngagementring"></div>');
	}
	// prepare the data
	var source =
	{
	...
	$("#jqxEngagementring").jqxGrid(
	{
		...
		columns: [
			...
			{ text: 'CAD Code', type: 'string', datafield: 'cad_code', width:'10%' },
			...
			{ text: 'Head Settings', type: 'string', datafield: 'head_title', width:'10%' },
			{ text: 'Claw Termination', type: 'string', datafield: 'claw_title', width:'10%' },
			{ text: 'Setting Height', type: 'string', datafield: 'setting_height', width:'10%' },
			{ text: 'Metal Thickness', type: 'string', datafield: 'metal_thickness', width:'10%' },
	...
	$("#jqxEngagementring").bind("sort", function (event) {
	   ...
	   //$.get(sortUrl, function(source){
	    	//$("#jqxEngagementring").jqxGrid('databind', source);
	    	$("#jqxEngagementring").jqxGrid('updatebounddata', 'filter');
	   //});
	});
...
function getEngagementringsFromGridView(recordsPerPage, columnList, keyword){
	...
	if(colList.length>5){
		for(i = 0; i < colList.length; i++){
			...
			if(colList[i] == 'metal_thickness'){
				gridColumnList.push({ text: 'Metal Thickness', type: 'string', datafield: 'metal_thickness', width:'10%' });
			}
			if(colList[i] == 'price'){
		...
		$("#jqxEngagementring").jqxGrid({columns:gridColumnList});
	}else{
	...
	if(colList.length == gridColumnList.length){
		...
		//$("#jqxEngagementring").next('.pagerHTML').html($('#pagerjqxEngagementring'));
		...
		if(errors == 0){
			...
			$.post(url, data, function(response){
				if(response > 0){
					...
					$('#metal_thickness_engagering').text($('#metal_thickness').val());
