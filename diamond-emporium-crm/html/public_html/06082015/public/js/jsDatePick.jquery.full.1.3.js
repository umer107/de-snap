...
g_l["DAYS_3"] = ["Su","Mo","Tu","We","Th","Fr","Sa"];
...
g_currentDateObject.highlightPattern = 0;
g_currentDateObject.endOnDate = 0;
...
JsDatePick.prototype.populateMainBox = function(aMainBox){
	...
	while (oDay.getMonth() == cmpMonth){
		...
		if(oDay.getDay() == 0 || oDay.getDay() == 6){
			aDayDiv.setAttribute("weekend", 1);
		}
		
		if(this.highlightDate(oDay))
			aDayDiv.setAttribute("highlight",1);
		else
			aDayDiv.setAttribute("highlight",'');
...
JsDatePick.prototype.datePatternSelected = function(pattern, endOnDate){
	this.highlightPattern = pattern;
	this.endOnDate = endOnDate;
	this.repopulateMainBox();
};

JsDatePick.prototype.highlightDate = function(oDay){
	var selectedDate = this.getSelectedDay();
	var response = false;
	if(selectedDate){
		var selectedDateObj = new Date(selectedDate.year, selectedDate.month - 1, selectedDate.day);
		var oDayDateObj = new Date(oDay.getFullYear(), oDay.getMonth(), oDay.getDate());
		if(this.highlightPattern == 2){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime())
				response = true;
		}else if(this.highlightPattern == 3){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime() && (oDayDateObj.getDay() != 0 && oDay.getDay() != 6))
				response = true;
		}else if(this.highlightPattern == 4){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime() && oDayDateObj.getDay() == selectedDateObj.getDay())
				response = true;
		}else if(this.highlightPattern == 5){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime() && (oDayDateObj.getDay() == selectedDateObj.getDay() && ((oDayDateObj.getTime() - selectedDateObj.getTime()) % (86400000 * 14) == 0)))			
				response = true;
		}else if(this.highlightPattern == 6){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime() && oDayDateObj.getDate() == selectedDateObj.getDate())
				response = true;
		}else if(this.highlightPattern == 7){
			if(oDayDateObj.getTime() >= selectedDateObj.getTime() && (oDayDateObj.getMonth() == selectedDateObj.getMonth() && oDayDateObj.getDate() == selectedDateObj.getDate()))
				response = true;
		}
		
		if(this.endOnDate && oDayDateObj.getTime() > this.endOnDate.getTime()){
			response = false;
		}
	}
