function getDiamonds(recordsPerPage, keyword, searchParams){
	...
	$("#jqxDianonds").jqxGrid(
	{
		...
		columns: [
			...
			{ text: 'Color', type: 'string', datafield: 'color', width:'8%'},
...
function getDiamondFromGridView(recordsPerPage, columnList, keyword, searchParams){
	...
	if(colList.length > 6){
		for(i = 0; i < colList.length; i++){
			...
			if(colList[i] == 'color'){
				gridColumnList.push({ text: 'Color', type: 'string', datafield: 'color', width:'8%'});
			}
		...
		for(i = 0; i < colList.length; i++){
			...
			if(colList[i] == 'color'){
				gridColumnList.push({ text: 'Color', type: 'string', datafield: 'color', width:colWidthData+'%' });
			}
			if(colList[i] == 'measurement'){
...
function validateConsign(){
	...
	var password = $('#frm_consign #password').val();
	
	if(user_id == 0){
		$( '<p class="errorText">Please select a Consignment owner</p>' ).insertAfter( '#frm_consign #password' );
		errors++;
	}
