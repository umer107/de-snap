...
class CastTable
{
	 ...
	 */
	public function saveCastStep($data){
		try{
			...
			if($data['steps_completed'] == 1){
				...
				if($data_changes){
					$cadDesignTable->updateJobPacket(3, $data['steps_completed'], $data['job_id'], $data['milestone_id']);
					return $data_changes;
				}
			} else {
				$job_id = $data['job_id'];
				...
				if($data_changes){
					$cadDesignTable->updateJobPacket(3, $data['steps_completed'], $job_id, $data['milestone_id']);
					return $data_changes;
				}
			}
		}catch(\Exception $e){
	...
	public function fetchCast($job_id){
		try{
			...
			$select->from(array('proto' => 'de_milestone_cast'))
				   ->columns(array('*'))
