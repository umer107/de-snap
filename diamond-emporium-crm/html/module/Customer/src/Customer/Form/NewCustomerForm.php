...
class NewCustomerForm extends Form {
	...
	public function __construct($objServiceManager) {
		try{
			...
			$title->setAttributes(array('id' => 'title', 'options' => array_merge(array('' => 'Select'), $config['titles']), 'class' => 'dropdown'));
			$this->add($title);
			
			$gender = new \Zend\Form\Element\Select('gender');
			$gender->setLabel('Gender');
			$gender->setAttributes(array('id' => 'gender', 'options' => array_merge(array('' => 'Select'), $config['genders']), 'class' => 'dropdown'));
			$this->add($gender);
			...
			$mobile->setAttributes(array('id' => 'customer_mobile', 'class' => 'inputTxt', 'maxlength' => 10));
			$this->add($mobile);
			
			$email = new \Zend\Form\Element\Text('email');
			$email->setLabel('Email');
			$email->setAttributes(array('id' => 'customer_email', 'class' => 'inputTxt', 'maxlength' => 128));
			$this->add($email);
			...
			$address2->setAttributes(array('id' => 'customer_address2', 'class' => 'inputTxt'));
			$this->add($address2);
