...
class LeadForm extends Form {
	...
	public function __construct($objServiceManager) {
		try{
			...
			$gender->setAttributes(array('id' => 'gender', 'options' => array_merge(array('' => 'Select'), $config['genders']), 'class' => 'dropdown width60p'));
			$this->add($gender);
			...
			$lead_source->setAttributes(array('id' => 'lead_source',
					'options' => $objLeadsTable->fetchLeadSourceOptions(),
					'class' => 'width60p dropdown'));
			...
			$how_heard = new \Zend\Form\Element\Select('how_heard');
			$how_heard->setLabel('How Heard');
			$how_heard->setAttributes(array('id' => 'how_heard', 'options' => $objHowHeardTypesTable->fetchSelectOptions(), 'class' => 'dropdown'));
			$this->add($how_heard);
			...
			$priority->setAttributes(array('id' => 'priority', 'class' => 'inputTxt', 'maxlength' => 200, 'style' => 'color:red'));
			$this->add($priority);
