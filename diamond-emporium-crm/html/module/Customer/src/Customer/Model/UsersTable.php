...
class UsersTable
{
	...
	public function fetchUsersForTasks()
	{
		try{			
			...
			$select = new \Zend\Db\Sql\Select();
			$select->from('de_users')
				   ->columns(array('user_id', 'fullname' => $fullname, 'shortname' => $shortname, 'color'));
			$result = $this->tableGateway->selectWith($select)->toArray();
			
			$options = array('0' => 'Unassigned');
			foreach($result as $value){
				if($value['color'] != NULL){
					$colorCode = 'style="background-color:#'.$value['color'].'"';
				} else {
					$colorCode = '';
				}
				$options[$value['user_id']] = '<span '.$colorCode.'>'.strtoupper($value['shortname']).'</span>'.ucfirst($value['fullname']);
			}
			return $options;
		}catch(\Exception $e){
			\De\Log::logApplicationInfo ( "Caught Exception: " . $e->getMessage () . ' -- File: ' . __FILE__ . ' Line: ' . __LINE__ );
