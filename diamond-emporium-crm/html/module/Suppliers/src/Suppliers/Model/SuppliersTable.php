		...
		try{
			...
			if(!empty($keyword)){
				...
				$where->NEST->addPredicates(array(
						...
						new \Zend\Db\Sql\Predicate\Like('sup.address1', "%$keyword%"),
			...
			if(!empty($filter['filter_services']) && is_array($filter['filter_services'])) {
				$in = implode(', ', $filter['filter_services']);
				$select->where->expression("sup.id IN (SELECT supplier_id FROM de_supplier_services WHERE service_id IN ($in))");
			}
//			\De\Log::logApplicationInfo ( "SQL: " . $select->getSqlString() . ' -- File: ' . __FILE__ . ' Line: ' . __LINE__ );
	...
	/**
	 * Fetch count of emails for the supplier
	 * $customer_id, primary key of customer table
	 */
	public function fetchEmailCount($supplier_id){
		try{
			$select = new \Zend\Db\Sql\Select();
			...
			$select->from(array('email' => 'de_milestone_email'))
				   ->columns(array('email_count' => $email_count))
				   ->where("email.supplier_id = $supplier_id");			
