...
class EngagementringInventoryForm extends Form {
	...
	public function __construct($objServiceManager) {
		try{
			...
			$band_width_top = new \Zend\Form\Element\Text('band_width_top');
			$band_width_top->setLabel('Band Width Top');
			$band_width_top->setAttributes(array('id' => 'band_width_top', 'class' => 'inputTxt', 'maxlength' => 64, 'data-numeric' => 'yes'));
			$this->add($band_width_top);
			
			$band_width_bottom = new \Zend\Form\Element\Text('band_width_bottom');
			$band_width_bottom->setLabel('Band Width Bottom');
			$band_width_bottom->setAttributes(array('id' => 'band_width_bottom', 'class' => 'inputTxt', 'maxlength' => 64, 'data-numeric' => 'yes'));
			$this->add($band_width_bottom);
			
			$band_thickness_top = new \Zend\Form\Element\Text('band_thickness_top');
			$band_thickness_top->setLabel('Band Thickness Top');
			$band_thickness_top->setAttributes(array('id' => 'band_thickness_top', 'class' => 'inputTxt', 'maxlength' => 64, 'data-numeric' => 'yes'));
			$this->add($band_thickness_top);

			$band_thickness_bottom = new \Zend\Form\Element\Text('band_thickness_bottom');
			$band_thickness_bottom->setLabel('Band Thickness Bottom');
			$band_thickness_bottom->setAttributes(array('id' => 'band_thickness_bottom', 'class' => 'inputTxt', 'maxlength' => 64, 'data-numeric' => 'yes'));
			$this->add($band_thickness_bottom);
