...
class WeddingringController extends AbstractActionController
{
	...
	public function weddingringdetailsAction(){
		try{
			// Write your code here
			$sm = $this->getServiceLocator();
			$canDelete = $sm->get('ControllerPluginManager')->get('AuthPlugin')->checkResource($sm->get('AuthService'), 'Inventory\Controller\Weddingring::deleteadditional');
				
			$config = $sm->get('Config');
			$id = $this->params('id');
			$additionalLookup = array();
			$WeddingringTable = $sm->get('Inventory\Model\WeddingringTable');
			...
			return array(
					'recordsPerPage' => $config['recordsPerPage'], 'viewData' => $WeddingringData,
					'ringTypesLookup' => $objLookupTable->fetchRingTypeOptions(),
					'metalTypesLookup' => $objLookupTable->fetchMetalTypeOptions(),
					'profileTypesLookup' => $objLookupTable->fetchProfileOptions(),
					'finishTypesLookup' => $objLookupTable->fetchFinishOptions(),
					'fitoptionsTypesLookup' => $objLookupTable->fetchFitOptions(),
