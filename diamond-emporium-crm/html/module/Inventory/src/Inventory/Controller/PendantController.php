...
class PendantController extends AbstractActionController
{
	...
	public function pendantdetailsAction(){
		try{
			// Write your code here
			$sm = $this->getServiceLocator();
			$canDelete = $sm->get('ControllerPluginManager')->get('AuthPlugin')->checkResource($sm->get('AuthService'), 'Inventory\Controller\Pendant::deletependant');

			$config = $sm->get('Config');
			$id = $this->params('id');
			$additionalLookup = array();
			$PendantTable = $sm->get('Inventory\Model\PendantTable');
			...
			return array(
					'recordsPerPage' => $config['recordsPerPage'], 'viewData' => $PendantData, 'ringTypesLookup' => $objLookupTable->fetchPendantStyleOptions(), 'fetchHeadSettingsOptions' => $objLookupTable->fetchHeadSettingsOptions(), 'metalTypesLookup' => $objLookupTable->fetchMetalTypeOptions(), 'profileTypesLookup' => $objLookupTable->fetchProfileOptions(), 'additionalLookup' => $additionalLookup, 'addtionalData' => $addtionalData, 'ConsignData' => $ConsignData,
