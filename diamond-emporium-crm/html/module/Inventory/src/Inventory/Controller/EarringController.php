...
class EarringController extends AbstractActionController
{
	...
	public function earringdetailsAction(){
		try{
			// Write your code here
			$sm = $this->getServiceLocator();
			$canDelete = $sm->get('ControllerPluginManager')->get('AuthPlugin')->checkResource($sm->get('AuthService'), 'Inventory\Controller\Earring::deleteearring');
				
			$config = $this->getServiceLocator()->get('Config');
			$id = $this->params('id');
			$additionalLookup = array();
			$EarringTable = $sm->get('Inventory\Model\EarringTable');
