...
class EngagementringController extends AbstractActionController
{
	...
	public function engagementringdetailsAction(){
		try{
			// Write your code here
			$sm = $this->getServiceLocator();
			$canDelete = $sm->get('ControllerPluginManager')->get('AuthPlugin')->checkResource($sm->get('AuthService'), 'Inventory\Controller\Engagementring::deleteengagementring');
			
			$config = $this->getServiceLocator()->get('Config');
			$id = $this->params('id');
			$additionalLookup = array();
			$EngagementringTable = $sm->get('Inventory\Model\EngagementringTable');
			...
			return array(
					...
					'viewData' => $EngagementringData,
					'ringTypesLookup' => $objLookupTable->fetchEngagementRingTypeOptions(),
					'metalTypesLookup' => $objLookupTable->fetchMetalTypeOptions(),
					'profileTypesLookup' => $objLookupTable->fetchProfileOptions(),
					'additionalLookup' => $additionalLookup,
					'addtionalData' => $addtionalData,
					'ConsignData' => $ConsignData,
					'fetchHeadSettingsOptions' => $objLookupTable->fetchHeadSettingsOptions(),
					'fetchClawTerminationOptions' => $objLookupTable->fetchClawTerminationOptions(),
					'canDelete' => $canDelete
			);
		}catch(Exception $e){
			\De\Log::logApplicationInfo ( "Caught Exception: " . $e->getMessage () . ' -- File: ' . __FILE__ . ' Line: ' . __LINE__ );
