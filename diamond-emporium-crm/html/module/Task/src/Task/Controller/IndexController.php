...
class IndexController extends AbstractActionController
{
    ...
    public function createtaskAction()
    {
    	try{
			...
			if($request->isPost()){
				...
				$data = array('task_title' => $posts['task_title'], 'due_date' => 'No Due Date',
				...
				$taskDetails = $tasksTable->fetchTaskDetails($task_id);
				
				if(!empty($taskDetails['lead_name']))
					$data['task_created_by_fullname'] = 'Lead >> '.$taskDetails['lead_name'];
				elseif(!empty($taskDetails['opportunity_name']))
					$data['task_created_by_fullname'] = 'Opportunity >> '.$taskDetails['opportunity_name'];
				elseif(!empty($taskDetails['customer_name']))
					$data['task_created_by_fullname'] = 'Customer >> '.$taskDetails['customer_name'];
	...
	public function gettaskdetailsAction()
    {
		try{
			...
			if($request->isPost()){
				...
				$taskData['task_assigned_shortname'] = strtoupper($taskData['task_assigned_shortname']);
				
				$filteredSubjects = array();
				if($taskData['task_category'] > 0){
					$tasksSubjectTable = $sm->get('Task\Model\TasksSubjectTable');
					$subjectList = $tasksSubjectTable->fetchAll();
					foreach($subjectList as $data){
						if($data['subject_category_id'] == $taskData['task_category']){
							$filteredSubjects[$data['subject_id']] = $data['subject_title'];
						}
					}
				}
				
				$taskData['task_subcatlist'] = $filteredSubjects;
				
				if(!empty($taskData['lead_name']))
					$taskData['task_created_by_fullname'] = 'Lead >> '.$taskData['lead_name'];
				elseif(!empty($taskData['opportunity_name']))
					$taskData['task_created_by_fullname'] = 'Opportunity >> '.$taskData['opportunity_name'];
				elseif(!empty($taskData['customer_name']))
					$taskData['task_created_by_fullname'] = 'Customer >> '.$taskData['customer_name'];
	...
	public function updateindividualAction()
    {
		try{
			...
			if($request->isPost()){
				...
				if($data['column_title'] == "task_category"){
					$updateData['task_subject'] = 0;
				}
	...
	public function gettaskhistorydetailsAction()
    {
		try{
			...
			if($request->isPost()){
				...
				/*if(!empty($taskHistoryData['lead_name']))
					$taskHistoryData['task_created_by_fullname'] = 'Lead >> '.$taskHistoryData['lead_name'];
				elseif(!empty($taskHistoryData['opportunity_name']))
					$taskHistoryData['task_created_by_fullname'] = 'Opportunity >> '.$taskHistoryData['opportunity_name'];
				elseif(!empty($taskHistoryData['customer_name']))
					$taskHistoryData['task_created_by_fullname'] = 'Customer >> '.$taskHistoryData['customer_name'];*/
	...
	public function getsubjectsAction(){
		try{
			...
			if($request->isPost()){
				$posts = $request->getPost()->toArray();
				$task_category = $posts['task_category'];
				...
				if($task_category > 0){
					...
					$subjectList = $tasksSubjectTable->fetchAll();
					foreach($subjectList as $data){
						if($data['subject_category_id'] == $task_category){
							$filteredSubjects[$data['subject_id']] = $data['subject_title'];
						}
					}
				}
				echo json_encode($filteredSubjects);
				exit;
			}			
		}catch(Exception $e){
			\De\Log::logApplicationInfo ( "Caught Exception: " . $e->getMessage () . ' -- File: ' . __FILE__ . ' Line: ' . __LINE__ );
